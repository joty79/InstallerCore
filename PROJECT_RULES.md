...
- Guardrail/rule: `profiles\SystemTools.json` owns the desktop-background SafeMode menu under `HKCU\Software\Classes\DesktopBackground\Shell\SystemTools\shell\SafeModeOptions` with visible label `Power Options`. Generated installs must deploy `SafeMode.ps1`, `NormalMode.ps1`, their launchers, and the SafeMode icon, then verify the Safe/Normal/Sleep commands. Clean the earlier `PowerMenu` key because Explorer did not render it despite correct registry readback.
